...
use Vendi\Shared\utils_exception;
...
class _bash_installer_base extends Command
{
    ...
    private int $_next_command_timeout = 100;

    private bool $_in_multi_command = false;

    private bool $_warn_about_long_running_command = false;
    ...
    {
        throw new RuntimeException('Child classes must implement the method test_requirements.');
    }

    protected function test_install(bool $quiet = false): bool
    {
        throw new RuntimeException('Child classes must implement the method test_install.');
    }

    final protected function set_next_command_timeout(int $seconds): void
    {
    ...
    final protected function reset_next_command_timeout(): void
    {
    ...
    protected function get_or_create_io(InputInterface $input = null, OutputInterface $output = null): SymfonyStyle
    {
        if (!$this->_io) {
            if (!$input || !$output) {
                throw new RuntimeException('You must either initialize IO elsewhere or provide Input and Output to do so here.');
            }
    ...
    protected function _run_multiple_commands_with_working_directory(array $commands, string $working_directory = null, bool $quiet = false): bool
    {
    ...
    protected function add_apt_key(string $key_server, string $key, bool $quiet = false): bool
    {
        ...
        $io->success("Successfully imported key server ${key_server} with key ${key}");
